...
findf_regex_f* intern__findf__init_regex(void)
{
  ...
  if ((to_init->sub_match = malloc(FRE_MAX_SUB_MATCHES * sizeof(int*))) == NULL){
    intern_errormesg("Malloc");
    goto cleanup;
  }
  for (i = 0; i < FRE_MAX_SUB_MATCHES; i++){
  /* The magical '2' stands for the begining of sub-match and ending of sub-match positions, respectively. */
    if ((to_init->sub_match[i] = calloc(2, sizeof(int))) == NULL){
      intern_errormesg("Calloc");
      goto cleanup;
    }
  }
				    ...
				    size_t *buf_len)
{
  ...
  case 'D':
    ...
    break;
  default:
    /* Look for back-references, '\1 to 8', \9 being reserved internaly. */
    if (isdigit(token)){
      if (token == '9') {
	intern_errormesg("Valid back-references are from 1 to 8 inclusively");
	return ERROR;
      }
      /* 
       * Increment the token by one. (\1 becomes \2 etc).
       * Remember back-ref \1 is used internaly for substitutions.
       */
      buffer[(*buf_ind)++] = token + 1;
    }
    /* Else assume an already supported escape sequence. */
...
# define STOKEN pattern[token_ind]
  ...
  /* 
   * Start by pushing an opening parenthese to the very begining of 
   * the operation's match pattern (pattern1).
   * We'll push a closing brace once pattern1 has been completed.
   * 
   * I think this function is vulnerable to buffer overflow. <----- 
   * 
   */
  intern__findf__push('(', freg_object->pat_storage->pattern1, &pattern_tos);
  ...
  while(1){
	...
	/* Push a closing parenthese to pattern1 now to capture the sub-match. */
	intern__findf__push(')', freg_object->pat_storage->pattern1, &pattern_tos);
			    ...
			    char * filename)
{
  ...
  /*
   * Sub-matches' positions are saved into the pattern's freg_object
   * in cases where backrefs are used in a substitution operation.
   * Substitution operations' first job is to call _match_op() (this function).
  ...
  if (regexec(freg_object->pattern, filename,
    ...
    /* Clear previous sub-matches. */
    for (i = 0; i < FRE_MAX_SUB_MATCHES; i++){
      freg_object->sub_match[i][0] = -1;
      freg_object->sub_match[i][1] = -1;
    }
    /* Register current sub-matches. */
    i = 0;
    while(pmatch_array[i].rm_so != -1) {
      freg_object->sub_match[sub_match_pos][0] = pmatch_array[i].rm_so;
      freg_object->sub_match[sub_match_pos++][1] = pmatch_array[i++].rm_eo;      
    }
